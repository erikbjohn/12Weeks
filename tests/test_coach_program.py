...
def test_keeps_valid_equipment_exercise():
    clean, dropped = validate_program(
        {0: [{"exercise": "Barbell Bench Press", "sets": 4, "reps": "5", "weight": 185, "why": "main"}]},
...
def test_keeps_bodyweight_regardless_of_equipment():
    clean, dropped = validate_program(
        {0: [{"exercise": "Push-Ups", "sets": 3, "reps": "15", "weight": None, "why": "x"}]},
        CATALOG, set())
...
def test_clamps_insane_set_counts():
    clean, _ = validate_program(
        {0: [{"exercise": "Push-Ups", "sets": 99, "reps": "10", "weight": None, "why": "x"}]},
        CATALOG, set())
...
def test_day_index_coerced_to_int():
    clean, _ = validate_program(
        {"4": [{"exercise": "Box Jump", "sets": 3, "reps": "5", "weight": 0, "why": "x"}]},
        CATALOG, set())
