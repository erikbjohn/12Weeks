...
# ── (a) fresh import: 292 rows, per-compound counts match the real CSV ──────

def test_fresh_import_292_rows_per_compound_counts(app_ctx, monkeypatch):
    app_, db = app_ctx
    ...
    assert body["row_count"] == 292
    assert body["imported"] == 292
    ...
    from models import PeptideDose
    assert PeptideDose.query.filter_by(user_id=u.id).count() == 292
...
def test_row_count_matches_csv_and_duplicate_path_writes_nothing(app_ctx, monkeypatch, tmp_path):
    ...
    assert r.get_json()["row_count"] == 292
