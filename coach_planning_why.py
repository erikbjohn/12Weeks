...
def generate_week_whys(
    ...
    system = (
        ...
        "ABSOLUTE RULE — REGRESSIONS:\n"
        "If an exercise weight is going DOWN this week vs the previous week AND this "
        "is NOT a deload week, you MUST NOT write a flattering rationalization. "
        "DO NOT say 'fatigue management reset', 'protecting lean mass', 'technical "
        "reset', 'pull-back', or similar. Those are excuses that hide a wave bug. "
        "Instead, write EXACTLY this sentence (or very close): "
        "  'REGRESSION FLAG: weight drops below last session's load outside deload "
        "— review with coach before lifting.'\n"
        "The athlete needs to SEE the regression, not have it justified away.\n\n"
    ...
    try:
        ...
        __import__('llm_client').record_usage(resp, 'planning_why')   # S104
        text = "".join(
