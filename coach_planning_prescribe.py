...
def generate_week_prescriptions(
    ...
    system = (
        ...
        "ABSOLUTE RULES:\n"
        "0. COVER EVERY EXERCISE in the template below — return one entry per "
        "   listed exercise, keyed exactly `<day>:<order>:<name>`. There is NO "
        "   engine fallback: any exercise you omit is left UNPLANNED for the "
        "   athlete. For bodyweight/plyometric lifts return weight null (or 0) "
        "   WITH a why — that still counts as covered. Omitting an exercise is "
        "   a hard fail.\n"
        "1. NEVER prescribe below the athlete's TOP SET in the last 4 weeks "
        "   unless this is an explicit deload week (4, 8, or 12). If they hit "
        "   145×5 last week, do not prescribe 140 this week. Period.\n"
        "2. Compound lifts (squat, deadlift, bench, press, row) progress +5 to "
        "   +10 lb/week when reps are being hit clean — IN EVERY phase, "
        "   including Phase 3 (weeks 9-11). There is NO Phase-3 hold; keep "
        "   adding load as long as the athlete keeps earning it.\n"
        "3. Accessory/isolation work (curls, raises, face pulls) progress +2.5 lb "
        "   per session when the athlete hits target reps — every phase.\n"
        "4. Bodyweight and plyometric (Box Jump, etc.): no weight prescription "
        "   needed — return 0 for target_weight.\n"
        "5. Deload weeks (4, 8, AND 12): 70-85% of prior week's working weight. "
        "   Week 12 is a deload week, not a hold and not a taper-to-nothing.\n"
        "6. Equipment-aware: if template says 'DB Bench Press' but the athlete "
    ...
    try:
        ...
        __import__('llm_client').record_usage(resp, 'planning_prescribe')   # S104
        text = "".join(
